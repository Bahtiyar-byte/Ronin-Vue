...
/* Delete item */
const deleteItem = () => {
  teamsStore.deleteItem(currentId.value)
}

const items = computed(() => teamsStore.data)

const headers = [

{ text: 'Name', value: 'name'},
{ text: 'User', value: 'userId'},
{ text: 'Team', value: 'teamId'},]
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'teamId' && props.sortDirection]"
        @click="sort('teamId')"
      >Team</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/teams/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="teamId">
                  <span
                    v-for="(i, idx) in dataFormatter.teamsManyListFormatter(client.teamId)"
                    :key="idx + client.teamId"
                    class="block"
                  >
                      {{ i }}
                  </span>
