...
/* Delete item */
const deleteItem = () => {
  contractsStore.deleteItem(currentId.value)
}

const items = computed(() => contractsStore.data)
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'signedDate' && props.sortDirection]"
        @click="sort('signedDate')"
      >Signed Date</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/contracts/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="signedDate">
                {{ dataFormatter.dateFormatter(client.signedDate) }}
              </td>
