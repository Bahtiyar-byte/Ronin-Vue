...
export const useMilestonesStore = defineStore('milestones', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`milestones${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/milestones', { data: payload });
        this.showNotification('Milestones has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/milestones/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Milestones has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
