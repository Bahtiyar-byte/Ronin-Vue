...
export const useRolesStore = defineStore('roles', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/roles', { data: payload });
        this.showNotification('Roles has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/roles/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Roles has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
