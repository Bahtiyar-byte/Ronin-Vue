...
export const useAddressesStore = defineStore('addresses', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`addresses${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/addresses', { data: payload });
        this.showNotification('Addresses has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/addresses/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Addresses has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
