...
export const usePermissionsStore = defineStore('permissions', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`permissions${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/permissions', { data: payload });
        this.showNotification('Permissions has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/permissions/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Permissions has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
