...
export const useAmendmentsStore = defineStore('amendments', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`amendments${query || (id ? `/${id}` : '')}`);
    ...
    async deleteItem(id) {
      try {
        await axios.delete(`/amendments/${id}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/amendments', { data: payload });
        this.showNotification('Amendments has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/amendments/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Amendments has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
