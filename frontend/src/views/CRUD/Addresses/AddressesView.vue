...
import TableSampleAddresses from '@/components/Addresses/TableSampleAddresses.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Street', title: 'street'},{label: 'Suite/Apt/Unit', title: 'suite_apt_unit'},{label: 'City', title: 'city'},{label: 'Zip', title: 'zip'},{label: 'Country', title: 'country'},

          {label: 'Contact ', title: 'contactId'},{label: 'Job ', title: 'jobId'},{label: 'Created By', title: 'createdBy'},];
...
const getAddressesCSV = async () => {
  const response = await axios({url: '/addresses?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'addressesCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('addresses/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    addressesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Addresses'
    : 'http://localhost:8080/api-docs/#/Addresses';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Addresses</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for addresses</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleAddresses checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
