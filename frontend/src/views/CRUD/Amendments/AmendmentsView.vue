...
import TableSampleAmendments from '@/components/Amendments/TableSampleAmendments.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Description', title: 'description'},
...
const getAmendmentsCSV = async () => {
  const response = await axios({url: '/amendments?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'amendmentsCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('amendments/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    amendmentsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Amendments'
    : 'http://localhost:8080/api-docs/#/Amendments';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Amendments</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for amendments</a>

    <CardBox
      class="mb-6"
      title="Amendments table"
      has-table
    >
      <TableSampleAmendments checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
