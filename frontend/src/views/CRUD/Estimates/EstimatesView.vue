...
import TableSampleEstimates from '@/components/Estimates/TableSampleEstimates.vue'
...
const filtersList = [{label: 'Description', title: 'description'},{label: 'Additional Notes', title: 'additionalNotes'},
          ...
          {label: 'Job Id', title: 'jobId'},{label: 'Contact Id', title: 'contactId'},{label: 'Template ', title: 'templateId'},];
...
const getEstimatesCSV = async () => {
  const response = await axios({url: '/estimates?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    estimatesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Estimates'
    : 'http://localhost:8080/api-docs/#/Estimates';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Estimates</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for estimates</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleEstimates checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
