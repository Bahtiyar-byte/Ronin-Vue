...
import TableSampleTemplates from '@/components/Templates/TableSampleTemplates.vue'
...
const filtersList = [{label: 'Name', title: 'name'},{label: 'Description', title: 'description'},
          ...
          {label: 'Material Cost', title: 'materialCost', number: 'true'},{label: 'Labor Cost', title: 'laborCost', number: 'true'},{label: 'Markup', title: 'markup', number: 'true'},{label: 'Profit Margin', title: 'profitMargin', number: 'true'},{label: 'Total Price', title: 'totalPrice', number: 'true'},
          ];
...
const getTemplatesCSV = async () => {
  const response = await axios({url: '/templates?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'templatesCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('templates/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    templatesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Templates'
    : 'http://localhost:8080/api-docs/#/Templates';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Templates</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for templates</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleTemplates checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
