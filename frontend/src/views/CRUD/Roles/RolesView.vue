...
import TableSampleRoles from '@/components/Roles/TableSampleRoles.vue'
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    rolesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Roles'
    : 'http://localhost:8080/api-docs/#/Roles';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Roles</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for roles</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleRoles checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
