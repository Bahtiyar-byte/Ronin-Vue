...
import TableSampleJobs from '@/components/Jobs/TableSampleJobs.vue'
...
const filtersList = [{label: 'Name', title: 'Name'},{label: 'Description', title: 'description'},
          ...
          {label: 'Contact ', title: 'contactId'},{label: 'Order ', title: 'orderId'},{label: 'Assigned User ', title: 'assignedUserId'},{label: 'Created By', title: 'createdBy'},{label: 'Updated By', title: 'updatedBy'},];
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    jobsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Jobs'
    : 'http://localhost:8080/api-docs/#/Jobs';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Jobs</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for jobs</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleJobs checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
