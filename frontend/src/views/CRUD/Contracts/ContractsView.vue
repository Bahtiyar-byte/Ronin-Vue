...
import TableSampleContracts from '@/components/Contracts/TableSampleContracts.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Name', title: 'name'},{label: 'Body', title: 'body'},
...
const getContractsCSV = async () => {
  const response = await axios({url: '/contracts?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'contractsCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('contracts/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    contractsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Contracts'
    : 'http://localhost:8080/api-docs/#/Contracts';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Contracts</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for contracts</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleContracts checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
