...
onMounted(() => {
  const fetchData = async () => {
    ...
    const { data: { count: emails } } = await axios.get('/emails/count');
    const { data: { count: chats } } = await axios.get('/chats/count');
    ...
    counts.chats = chats;
    counts.appointments = appointments;
    counts.tasks = tasks;
