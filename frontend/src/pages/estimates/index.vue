...
<template>
  <ItemsManage v-model:items-per-page="pagination.itemsPerPage" v-model:search-query="searchQuery"
    items-title="Estimates" :search-settings="{ placeholder: 'Search estimates' }" :breadcrumbs="[
      {
        title: 'Home',
        to: { name: 'root' },
      },
    ...
    <template #buttons>
      <VBtn :to="{ name: 'estimates-create' }" prepend-icon="tabler-plus">
        Create estimate
      </VBtn>
    </template>
    ...
    <template #table>
      <VDataTable v-model="selectedItems" v-model:sort-by="sortBy" :items="items" :headers="headers"
        ...
        <template #item.actions="{ item }">
          <IconBtn :to="{ name: 'estimates-details-id', params: { id: item.id } }" title="View">
            <VIcon icon="tabler-eye" />
          </IconBtn>
          <IconBtn :to="{ name: 'estimates-update-id', params: { id: item.id } }" title="Edit">
