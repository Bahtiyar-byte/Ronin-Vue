...
module.exports = class MilestonesDBApi {
  ...
  static async bulkImport(data, options) {
    ...
    // Bulk create items
    const milestones = await db.milestones.bulkCreate(milestonesData, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    const milestones = await db.milestones.findOne({ where }, { transaction });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.Name) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('milestones', 'Name', filter.Name),
        };
      }
    ...
    let { rows, count } = options?.countOnly
      ? {
          rows: [],
          count: await db.milestones.count({
            where,
            include,
            distinct: true,
      ...
      : await db.milestones.findAndCountAll({
          where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
  ...
  static async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('milestones', 'id', query),
        ],
      };
    }
