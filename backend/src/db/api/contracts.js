...
module.exports = class ContractsDBApi {
  static async create(data, options) {
    ...
    await contracts.setContactId(data.contactId || null, {
      transaction,
    });

    await contracts.setJobId(data.jobId || null, {
      transaction,
    });

    await contracts.setCreatedBy(data.createdBy || null, {
      transaction,
    });
  ...
  static async bulkImport(data, options) {
    ...
    const contractsData = data.map((item, index) => ({
      ...
      body: item.body || null,
      signedDate: item.signedDate || null,
  ...
  static async findBy(where, options) {
    ...
    output.contactId = await contracts.getContactId({
      transaction,
    });

    output.jobId = await contracts.getJobId({
      transaction,
    });

    output.createdBy = await contracts.getCreatedBy({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.signedDateRange) {
        const [start, end] = filter.signedDateRange;
